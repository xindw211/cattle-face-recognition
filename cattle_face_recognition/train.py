...
def main(args):
    ...
    name = get_config_name(args.config)
    ...
    if cfg.using_wandb:
        ...
        # Sign in to wandb
        try:
            wandb.login(key=cfg.wandb_key)# 登录wandB
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for batch_idx, (img, local_labels) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                loss_list.append(loss_am.avg)

        epoch_loss = np.mean(loss_list)
        loss_list.clear()
        if not os.path.exists('work_dirs/{}/loss_log'.format(name)):
                os.makedirs('work_dirs/{}/loss_log'.format(name))

        with open('work_dirs/{}/loss_log/log.txt'.format(name), 'a') as f:
            val_list = [
                epoch,
                epoch_loss
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, help="py config file",default='configs/cow_config')
